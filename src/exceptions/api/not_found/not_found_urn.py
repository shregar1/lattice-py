"""NotFoundURNException — raised when a resource with specified URN is not found."""

from constants import ExceptionCode, ExceptionKey
from .abstraction import INotFoundException


class NotFoundURNException(INotFoundException):
    """
    Raised when a resource with specified URN is not found.
    ...
    code = ExceptionCode.NOT_FOUND_URN
    key = ExceptionKey.NOT_FOUND_URN

    @property
    def name(self) -> str:
        """Returns the class name."""
        return "ExceptionName.NotFoundURNException"
