...
"""Delete application repository service module."""
...
from dtos import DeleteApplicationServiceDTO
...
class DeleteApplicationService(IApplicationRepositoryService):
    """Application service for delete operation on application repository."""
    ...
    async def run(self, request: DeleteApplicationServiceDTO) -> DTOLayer:
        ...
        try:
            ...
            if request.id:
                ...
                if not self.application_repository.exists_by_id_candidate_user_and_tenant(
                    application_id=request.id,
                    user_id=request.user_id,
                    tenant_id=request.tenant_id,
                ):
                    raise ForbiddenException("User does not have permission to delete this application resource.")

                if request.is_hard_delete:
                    return self.application_repository.hard_delete(id=request.id)
                return self.application_repository.soft_delete(id=request.id)
            return request
    ...
    def name(self) -> str:
        ...
        return Service.DELETE_APPLICATION_SERVICE
