"""Repository implementation for CompanyProfile."""
...
from models import Company, CompanyProfile, Tenant, User
from repositories.atomic import CompanyProfileRepository as AtomicCompanyProfileRepository
from utilities import Logger

from ..abstraction import ICompositeRepository


class CompanyProfileRepository(AtomicCompanyProfileRepository, ICompositeRepository):
    """Repository for CompanyProfile; extends AtomicCompanyProfileRepository."""
    ...
    def __init__(
        ...
        model: type[CompanyProfile] = Dependency(CompanyProfileDependency),
        ...
        **kwargs: Any,
        ...
        AtomicCompanyProfileRepository.__init__(
            self,
            urn=urn,
            tenant_id=tenant_id,
        ...
        self.company_profile_model = model
    ...
    def exists_by_id_user_and_tenant(
        ...
        try:
            result = (
                self.company_profile_model.exists(id=entity_id, tenant_id=tenant_id)
    ...
    def filter_by_tenant(self, tenant_id: int) -> List[CompanyProfile]:
        """Filter CompanyProfile records by tenant_id."""
        start_time = time.time()

        try:
            result = list(self.company_profile_model.find_many(tenant_id=tenant_id))
            end_time = time.time()
            self.logger.info(
