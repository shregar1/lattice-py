...
from models import Company, Project, Tenant, User
from repositories.atomic import ProjectRepository as AtomicProjectRepository
from utilities import Logger

from ..abstraction import ICompositeRepository


class ProjectRepository(AtomicProjectRepository, ICompositeRepository):
    """Repository for Project; extends AtomicProjectRepository."""
    ...
    def __init__(
        ...
        model: type[Project] = Dependency(ProjectDependency),
        ...
        **kwargs: Any,
        ...
        AtomicProjectRepository.__init__(
            self,
            urn=urn,
            tenant_id=tenant_id,
    ...
    def exists_by_id_user_and_tenant(
        ...
        try:
            result = (
                self.project_model.exists(id=entity_id, tenant_id=tenant_id)
    ...
    def filter_by_tenant(self, tenant_id: int) -> List[Project]:
        """Filter Project records by tenant_id."""
        start_time = time.time()

        try:
            result = list(self.project_model.find_many(tenant_id=tenant_id))
            end_time = time.time()
            self.logger.info(
    ...
    def filter_by_project_name(self, project_name: str) -> List[Project]:
        """Filter Project records by project_name."""
        start_time = time.time()

        try:
            result = list(self.project_model.find_many(project_name=project_name))
            end_time = time.time()
            self.logger.info(
                "Query execution succeeded for filter_by_project_name",
                start_time=start_time,
                end_time=end_time,
                duration_ms=(end_time - start_time) * 1000,
            ...
            end_time = time.time()
            self.logger.error(
                "Query execution failed for filter_by_project_name",
                start_time=start_time,
                end_time=end_time,
                duration_ms=(end_time - start_time) * 1000,
                details={"project_name": project_name},
