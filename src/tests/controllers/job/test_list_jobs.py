...
class TestListJobs:
    ...
    @staticmethod
    def test_list_jobs_responds(client):
    ...
    @staticmethod
    def test_list_jobs_response_is_json(client):
        response = client.get(API_PATH)
        assert isinstance(response.json(), dict)

    @staticmethod
    def test_list_jobs_with_query_filter(client):
        response = client.get(f'{API_PATH}?query=engineer')
        assert response.status_code in (200, 422, 500)
    ...
    @staticmethod
    def test_list_jobs_delete_not_allowed(client):
